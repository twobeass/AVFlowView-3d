...
export class DebugPanel {
  constructor(renderer, containerSelector = '#debug-panel-container') {
    ...
    this.isVisible = false; // Initialize as false so it starts hidden
  ...
  attachEventHandlers() {
    ...
    // Note: Visualization toggles are now controlled by TopBar
    // The debug state is updated via the updateDebugState() method
  ...
  /**
   * Update debug state from external source (e.g., TopBar)
   * @param {object} newState - New debug state object
   */
  updateDebugState(newState) {
    this.debugState = { ...this.debugState, ...newState };
  ...
  updateStatistics() {
    ...
    d3.select('#debug-layout-time').text(
      `${this.stats.layoutTime.toFixed(2)}ms`
    );
  }
  ...
  updateVisualization() {
    ...
    const debugGroup = this.renderer.g
      .append('g')
  ...
  highlightRoutingMethods(_group) {
    ...
    this.renderer.g.selectAll('.edge-path').each(function (d) {
      ...
      if (d._debugRoutingMethod === 'ELK') {
        ...
        edgePath.style('stroke', '#333').style('stroke-width', 3);
      }
    });
  }
  ...
  highlightParallelEdges(_group) {
    ...
    parallelGroups.forEach((edges, _key) => {
      if (edges.length > 1) {
        ...
        edges.forEach((edge) => {
          this.renderer.g
            .selectAll(`.edge-path[data-id="${edge.id}"]`)
            .style('stroke', color)
  ...
  inspectEdge(edgeId) {
    ...
    if (edge.sections && edge.sections[0]) {
      ...
      html +=
        '<div class="inspector-field"><strong>ELK Routing:</strong><ul class="bend-list">';
    ...
    if (!edgePath.empty()) {
      ...
      if (pathData) {
        ...
        if (pathPoints.length > 0) {
          html +=
            '<div class="inspector-field"><strong>Rendered Path:</strong><ul class="bend-list">';
  ...
  updateDiagnosticData(data) {
        ...
        if (edge.sections && edge.sections[0]) {
          ...
          if (srcPort && edge.sections[0].startPoint) {
            const deltaX = Math.abs(
              srcPort.absX - edge.sections[0].startPoint.x
            );
            const deltaY = Math.abs(
              srcPort.absY - edge.sections[0].startPoint.y
            );
    ...
    d3.select('#debug-diagnostic-data').html(
      `<pre class="diagnostic-pre">${diagnosticText}</pre>`
    );
  }
